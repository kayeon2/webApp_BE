...
public class BookController {
    ...
    public ResponseEntity<?> createBook(@AuthenticationPrincipal String userId, @RequestBody BookDTO dto) {
        try {
            ...
            List<BookEntity> entities = service.create(entity); // 서비스를 이용해 book 엔티티 생성
    ...
    @GetMapping
    public ResponseEntity<?> retrieveBookList(@AuthenticationPrincipal String userId, @RequestParam(required = false) String title) {
        ...
        if (title != null && !title.isEmpty()) {
            entities = service.retrieve(title);
        } else {
            entities = service.retrieveAll(userId);
        }
        List<BookDTO> dtos = entities.stream().map(BookDTO::new).collect(Collectors.toList()); // entity 리스트 -> dto 리스트 변환
        ResponseDTO<BookDTO> response = ResponseDTO.<BookDTO>builder().data(dtos).build(); // dto 리스트로 ResponseDTO 초기화
    ...
    public ResponseEntity<?> updateBook(@AuthenticationPrincipal String userId, @RequestBody BookDTO dto) {
        ...
        entity.setUserId(userId);
        List<BookEntity> entities = service.update(entity); // 서비스를 이용해 book 엔티티 수정
        List<BookDTO> dtos = entities.stream().map(BookDTO::new).collect(Collectors.toList()); // entity 리스트 -> dto 리스트 변환
        ResponseDTO<BookDTO> response = ResponseDTO.<BookDTO>builder().data(dtos).build(); // dto 리스트로 ResponseDTO 초기화
    ...
    public ResponseEntity<?> deleteBook(@AuthenticationPrincipal String userId, @RequestBody BookDTO dto) {
        try {
            ...
            List<BookEntity> entities = service.delete(entity); // 서비스를 이용해 book 엔티티 삭제
            List<BookDTO> dtos = entities.stream().map(BookDTO::new).collect(Collectors.toList()); // entity 리스트 -> dto 리스트 변환
            ResponseDTO<BookDTO> response = ResponseDTO.<BookDTO>builder().data(dtos).build(); // dto 리스트로 ResponseDTO 초기화
